...
namespace ThreeSoftECommAPI.Services.EComm.ProductServ
{
    public class ProductService : IProductService
    {
        ...
        public async Task<List<Product>> GetProductsAsync(Int64 subCategoryId, int status)
        {
            ...
            {
                case 1:
                    return await _dataContext.product.Where(c => c.SubCategoryId == subCategoryId).Where(x => x.status == 1).ToListAsync();
                case 0:
        ...
        {
            var query = await (from p in _dataContext.product where p.status == 1
        ...
        public async Task<ViewProductResponse> ViewProductAsync(long ProductId)
        {
            ...
            var prodColor = await _dataContext.ProductColors.Where(x => x.ProductId == ProductId).ToListAsync();
            var prodSize = await _dataContext.ProductSizes.Where(x => x.ProductId == ProductId).ToListAsync();
